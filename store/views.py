...
def homei(request):
    products = Product.objects.all().filter(is_available = True).order_by('-id')[0:8]
...
def shop(request,category_slug=None,subcategory_slug=None):
    ...
    if category_slug != None:                 
        ...
        if subcategory_slug != None:
            ...
            products = Product.objects.filter(sub_category =subcategories,is_available = True)
            paginator = Paginator(products,4)
            page= request.GET.get('page')
            paged_products =  paginator.get_page(page)
...
def product_details(request,category_slug,subcategory_slug,product_slug):
    try:
        print("my try")
        single_product = Product.objects.filter(category__slug=category_slug,sub_category__slug=subcategory_slug,slug=product_slug)
        related_products = Product.objects.filter(sub_category__slug=subcategory_slug)[0:4]
...
def addcart(request,product_id): 
    ...
    product = Product.objects.get(id=product_id)  # get the product  #getting session in cart   using cartid                                                    
    product_variation = []
    ...
    if is_cart_item_exists:
        ...
        if product_variation in existing_variation_list:
            ...
            if len(product_variation) > 0:              # checking product variation empty or not 
                    item.variations.clear()
                    item.variations.add(*product_variation) # cart_item.quantity += 1    # incresing quantinty in cart
                    print(item)
                    item.save()
